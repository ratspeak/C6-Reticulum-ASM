"""Direct QEMU tests for SX1262 command wrappers."""
...
LORA_ERR_SPI_TIMEOUT = 0xFFFFFFFC
SPI_OK = 0
SPI_ERR_TIMEOUT = 0xFFFFFFFD
GPIO_NSS_MASK = 1 << 0
GPIO_BUSY_MASK = 1 << 6
...
def _build_test_elf(tmp_path: Path, body: str, data: str = "") -> Path:
    ...
    harness = tmp_path / "sx1262_command_harness.S"
    ...
    elf = tmp_path / "sx1262_command_harness.elf"
    _run([
        ld_bin,
        "-nostdlib",
        "-static",
...
TEST_DATA = """
        .section .data.sx1262_command_test, "aw", @progbits
        .balign 4
payload3:
        .byte   0x01, 0x02, 0x03
request2:
...
def test_command_write_waits_busy_and_sends_opcode_payload(tmp_path: Path) -> None:
    body = (
        _call("spi_init")
        + _call("sx1262_command_write", 0x8A, "payload3", 3, 100)
...
def test_command_write_rejects_invalid_arguments(tmp_path: Path) -> None:
    body = (
        _call("spi_init")
        + _call("sx1262_command_write", 0x100, 0, 0, 100)
        + _call("sx1262_command_write", 0x8A, 0, 1, 100)
        + _call("sx1262_command_write", 0x8A, "payload3", 256, 100)
        + _call("sx1262_command_read", 0x1D, 0, 0, 0, 1, 100)
        + _call("sx1262_command_read", 0x1D, "request2", 2, "rx2", 254, 100)
    )
...
def test_command_write_busy_timeout_keeps_nss_idle(tmp_path: Path) -> None:
    body = (
        _call("spi_init")
        + _store_word("gpio_model_level", GPIO_BUSY_MASK)
        + _call("sx1262_command_write", 0x80, 0, 0, 2)
        + _load_word("gpio_model_level")
        + _load_word("spi_model_transfer_count")
    )
...
def test_command_write_spi_timeout_deasserts_nss(tmp_path: Path) -> None:
    body = (
        _call("spi_init")
        + _store_word("spi_model_force_timeout", 1)
        + _call("sx1262_command_write", 0x80, 0, 0, 100)
        + _load_word("gpio_model_level")
        + _load_word("spi_model_last_status")
    )
    assert _run_body(tmp_path, body, 4, TEST_DATA) == [
        SPI_OK,
        LORA_ERR_SPI_TIMEOUT,
        GPIO_NSS_MASK,
        SPI_ERR_TIMEOUT,
    ]


def test_command_read_clocks_request_and_copies_response(tmp_path: Path) -> None:
    body = (
        _call("spi_init")
        + _store_word("spi_model_rx_source", 0x11CCBBAA)
        + _store_byte("spi_model_rx_source", 4, 0x22)
        + _call("sx1262_command_read", 0x1D, "request2", 2, "rx2", 2, 100)
        + _load_word("spi_model_tx_log_len")
        + _load_word("spi_model_transfer_count")
        + _load_word("spi_model_tx_log")
        + _load_word("rx2")
