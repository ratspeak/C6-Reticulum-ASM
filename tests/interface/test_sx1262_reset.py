"""Direct QEMU tests for SX1262 reset helper."""
...
def _build_test_elf(tmp_path: Path, body: str) -> Path:
    ...
    harness = tmp_path / "sx1262_reset_harness.S"
    ...
    elf = tmp_path / "sx1262_reset_harness.elf"
    _run([
        ld_bin,
        "-nostdlib",
        "-static",
...
def test_reset_pulses_nrst_and_accepts_busy_low(tmp_path: Path) -> None:
    body = (
        _call("sx1262_reset", 3)
    ...
    assert _run_body(tmp_path, body, 4) == [
        LORA_OK,
        GPIO_BUSY_MASK | GPIO_NRST_MASK,
        GPIO_NRST_MASK,
        GPIO_NRST_MASK,
    ]


def test_reset_zero_budget_uses_default_when_busy_low(tmp_path: Path) -> None:
    body = _call("sx1262_reset", 0) + _load_word("gpio_model_level")
    assert _run_body(tmp_path, body, 2) == [
        LORA_OK,
        GPIO_NRST_MASK,
    ]


def test_reset_times_out_when_busy_stays_high(tmp_path: Path) -> None:
    body = (
        _store_word("gpio_model_level", GPIO_BUSY_MASK)
        + _call("sx1262_reset", 2)
        + _load_word("gpio_model_level")
    )
    assert _run_body(tmp_path, body, 2) == [
        0xFFFFFFFE,
