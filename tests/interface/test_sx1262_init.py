"""Direct QEMU tests for SX1262 static-profile init."""
...
def _build_test_elf(tmp_path: Path, body: str) -> Path:
    ...
    harness = tmp_path / "sx1262_init_harness.S"
    ...
    elf = tmp_path / "sx1262_init_harness.elf"
    _run([
        ld_bin,
        "-nostdlib",
        "-static",
...
def test_init_configures_static_profile_and_reads_status(tmp_path: Path) -> None:
    body = (
    ...
    assert _run_body(tmp_path, body, 10) == [
        LORA_OK,
        1,
        0x5A,
        LORA_OK,
        ...
        2,
        0x026300C0,
        GPIO_NSS_MASK | GPIO_DIO1_MASK | GPIO_BUSY_MASK | GPIO_NRST_MASK,
        GPIO_NSS_MASK | GPIO_NRST_MASK,
        GPIO_NSS_MASK | GPIO_NRST_MASK,
    ]


def test_init_zero_timeout_uses_default(tmp_path: Path) -> None:
    body = (
...
def test_init_busy_timeout_records_error_without_commands(tmp_path: Path) -> None:
    body = (
