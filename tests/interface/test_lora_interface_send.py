"""Direct QEMU tests for raw Reticulum packet TX through the LoRa interface."""
...
ASM_SOURCES = (
    "src/interface/lora/lora_interface_send.S",
...
def _build_test_elf(tmp_path: Path, body: str, data: str = "") -> Path:
    ...
    harness = tmp_path / "lora_interface_send_harness.S"
    ...
    elf = tmp_path / "lora_interface_send_harness.elf"
    _run([
        ld_bin,
        "-nostdlib",
        "-static",
...
TEST_DATA = """
        .section .data.lora_interface_send_test, "aw", @progbits
        .balign 4
packet3:
        .byte   0x11, 0x22, 0x33
"""
...
def test_lora_interface_send_transmits_after_interface_init(tmp_path: Path) -> None:
    body = (
        _store_word("spi_model_rx_source", 0x00010000)
        + _call("lora_interface_init", 100)
        + _call("lora_interface_send", "packet3", 3, 100)
        + _load_word("lora_interface_tx_len")
...
def test_lora_interface_send_requires_interface_init(tmp_path: Path) -> None:
    body = (
...
def test_lora_interface_send_records_delegated_argument_errors(tmp_path: Path) -> None:
    body = (
        _store_word("spi_model_initialized", 1)
        + _store_word("sx1262_model_initialized", 1)
        + _store_word("lora_interface_initialized", 1)
        + _call("lora_interface_send", 0, 3, 100)
        + _call("lora_interface_send", "packet3", 0, 100)
        + _call("lora_interface_send", "packet3", 254, 100)
        + _load_word("lora_interface_tx_len")
        + _load_word("lora_interface_last_status")
        + _load_word("spi_model_transfer_count")
    )
...
def test_lora_interface_send_records_radio_timeout(tmp_path: Path) -> None:
    body = (
