"""Direct QEMU tests for LXMF inbound dispatch."""
...
ASM_SOURCES = (
    "src/lxmf/lxmf_inbound_dispatch.S",
...
def _dispatch_payload(case: Case) -> bytes:
    if case.dispatch_override is not None:
        return case.dispatch_override
    packed = _packed(case)
    if case.mode == MODE_OPPORTUNISTIC:
        return packed[HASH_SIZE:]
    return packed


CASES = (
    Case("direct-valid"),
    Case("opportunistic-valid", mode=MODE_OPPORTUNISTIC),
    Case("duplicate-direct", repeat=True, expect_ret=1),
    Case("capacity-full", prefill_id=bytes([0xEE]) * 32, expect_ret=-8),
    Case("unknown-source", install_path=False, expect_ret=-7),
    Case("bad-signature", flip_sig=True, expect_ret=-5),
    Case("tampered-payload", payload=ALT_PAYLOAD, sign_payload=PAYLOAD, expect_ret=-5),
    Case("no-direct-link", link_established=0, expect_ret=-9),
    Case("short-direct", dispatch_override=bytes(PACKED_PREFIX - 1), expect_ret=-6),
    Case("short-opportunistic", mode=MODE_OPPORTUNISTIC, dispatch_override=bytes(PACKED_PREFIX - HASH_SIZE - 1), expect_ret=-6),
    Case("opportunistic-null-dest", mode=MODE_OPPORTUNISTIC, null_dest=True, expect_ret=-1),
    Case("null-payload", null_payload=True, expect_ret=-1),
    Case("invalid-mode", mode=7, expect_ret=-1),
    Case(
        "payload-overflow",
        dispatch_override=DEST + IDENTITY.hash + bytes(SIGNATURE_SIZE) + bytes(MAX_STAMPED + 1),
        expect_ret=-8,
    ),
)
...
def _harness(case: Case) -> str:
    dispatch_payload = _dispatch_payload(case)
    payload_ptr = "mv      a0, zero" if case.null_payload else "la      a0, dispatch_payload"
    dest_ptr = "mv      a2, zero" if case.null_dest else "la      a2, dest_hash"
    install_path = """
        la      t0, transport_path_table
        li      t1, 1
        sb      t1, TRANSPORT_PATH_OFF_VALID(t0)
        addi    a1, t0, TRANSPORT_PATH_OFF_DEST_HASH
        la      a0, source_hash
        li      a2, 16
        call    .Lcopy_bytes
        la      t0, transport_path_table
        addi    a1, t0, TRANSPORT_PATH_OFF_PUBLIC_KEY
        la      a0, source_public_key
        li      a2, 64
        call    .Lcopy_bytes
""" if case.install_path else ""
    prefill = ""
    if case.prefill_id is not None:
        prefill = f"""
        la      a0, prefill_id
        la      a1, lxmf_inbound_message_id
        li      a2, 32
        call    .Lcopy_bytes
        la      t0, lxmf_inbound_slot_valid
        li      t1, 1
        sb      t1, 0(t0)
"""
    repeat_call = ""
    if case.repeat:
        repeat_call = f"""
        {payload_ptr}
        li      a1, {len(dispatch_payload)}
        {dest_ptr}
        li      a3, {case.mode}
        li      a4, {case.link_established}
        call    lxmf_inbound_dispatch
"""
    return f"""
        .include "lxmf.S"
        .include "transport.S"
...
{repeat_call}
        ...
        call    .Lemit_sep
        la      a0, lxmf_inbound_dest_hash
        li      a1, 16
        call    .Lemit_bytes

        call    .Lemit_sep
        la      a0, lxmf_inbound_source_hash
        li      a1, 16
        call    .Lemit_bytes

        call    .Lemit_sep
        la      t0, lxmf_inbound_payload_len
        lw      a0, 0(t0)
        call    .Lemit_hex32
...
dispatch_payload:
        .byte   {_byte_list(dispatch_payload)}
dest_hash:
        .byte   {_byte_list(case.dest)}
source_hash:
        .byte   {_byte_list(case.source)}
source_public_key:
        .byte   {_byte_list(IDENTITY.public_key)}
prefill_id:
        .byte   {_byte_list(case.prefill_id or bytes(32))}
"""
...
def _build_test_elf(tmp_path: Path, asm: str) -> Path:
    ...
    harness = tmp_path / "lxmf_inbound_dispatch_harness.S"
    ...
    elf = tmp_path / "lxmf_inbound_dispatch.elf"
    _run([
        ld_bin,
        "-nostdlib",
        "-static",
...
def test_symbol_exists(artifacts: build.BuildArtifacts) -> None:
    assert build.symbol_address(artifacts.elf, "lxmf_inbound_dispatch") > 0


@pytest.mark.parametrize("case", CASES, ids=[c.name for c in CASES])
def test_lxmf_inbound_dispatch_qemu(tmp_path: Path, case: Case) -> None:
