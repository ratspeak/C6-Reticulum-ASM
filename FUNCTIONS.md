...
- Total functions registered: **109** (excludes placeholder rows like
  "(functions added when milestone N is activated)")
- Verified: 100 (milestone 1 software side + ENTIRE milestone-2 crypto stack: SHA-256 family + SHA-512 family + HMAC + HKDF + AES-256-CBC stack + 13 X25519 functions + 11 Ed25519 functions (scalar arith, point ops, scalarmult, compress/decompress, keypair, sign, verify — all match pyca/cryptography under QEMU; RFC 7748 §5.2/§6.1 + RFC 8032 §7.1 vectors plus tampering rejection) + production HMAC-DRBG-SHA-256 RNG (NIST SP 800-90A Rev. 1 §10.1.2; `rng_entropy` raw-source layer + `hmac_drbg_update` + `rng_init` + `rng_bytes` together discharge the canonical NIST CAVP DRBGVS COUNT=0 KAT symbolically; on TARGET_C6 the entropy source is the on-chip LPPERI hardware RNG) + milestone-3 identity/destination/announce TX helpers (`identity_create`, `identity_hash`, `destination_name_hash`, `destination_hash`, `announce_build`, `announce_send`) + milestone-4 qemu flash model (`flash_init`, `flash_read`, `flash_write_page`, `flash_erase_sector`) + milestone-4 identity persistence (`identity_save`, `identity_load`) + milestone-5 transport RX helpers (`announce_parse`, `announce_validate`, `transport_path_init`, `transport_path_update`, `transport_path_lookup`, `transport_process_announce`) — all under ADR-0009)
...
- Tested: 0
- In progress: 0
- Planned: 9 (`uart_isr` plus milestone-6 link functions:
  `link_request_build`, `link_request_parse`, `link_handshake_init`,
  `link_handshake_accept`, `link_derive_keys`, `link_session_encrypt`,
  `link_session_decrypt`, and `link_process_packet`)
...
| Function | Status | Owner | Depends-on | ADRs | Spec |
|----------|--------|-------|-----------|------|------|
| `link_request_build` | ☐ planned |  | `x25519_keypair`, `packet_serialize_header` | 0001, 0002, 0005, 0006, 0009 | [milestone-6](docs/milestones/milestone-6.md#link_request_build) |
| `link_request_parse` | ☐ planned |  | `packet_parse_header` | 0001, 0002, 0005, 0006, 0009 | [milestone-6](docs/milestones/milestone-6.md#link_request_parse) |
| `link_handshake_init` | ☐ planned |  | `link_request_build`, `x25519_keypair`, `clock_now_ms` | 0001, 0002, 0005, 0006, 0009 | [milestone-6](docs/milestones/milestone-6.md#link_handshake_init) |
| `link_handshake_accept` | ☐ planned |  | `link_request_parse`, `link_derive_keys`, `clock_now_ms` | 0001, 0002, 0005, 0006, 0009 | [milestone-6](docs/milestones/milestone-6.md#link_handshake_accept) |
| `link_derive_keys` | ☐ planned |  | `x25519_scalar_mult`, `hkdf_extract`, `hkdf_expand` | 0001, 0002, 0005, 0006, 0009 | [milestone-6](docs/milestones/milestone-6.md#link_derive_keys) |
| `link_session_encrypt` | ☐ planned |  | `aes256_cbc_encrypt`, `hmac_sha256`, `rng_bytes` | 0001, 0002, 0005, 0006, 0009 | [milestone-6](docs/milestones/milestone-6.md#link_session_encrypt) |
| `link_session_decrypt` | ☐ planned |  | `aes256_cbc_decrypt`, `hmac_sha256` | 0001, 0002, 0005, 0006, 0009 | [milestone-6](docs/milestones/milestone-6.md#link_session_decrypt) |
| `link_process_packet` | ☐ planned |  | `link_request_parse`, `link_handshake_accept`, `link_session_decrypt`, `transport_path_lookup` | 0001, 0002, 0004, 0005, 0006, 0009 | [milestone-6](docs/milestones/milestone-6.md#link_process_packet) |
