...
- Planned: Ed25519 Tier A formal model (`sc_reduce`, `sc_muladd`, `point_add`, `point_double`, `scalarmult`, `point_compress`, `point_decompress`, `field_pow_p5d8`, `keypair`, `sign`, `verify`); Binsec/Rel ct backfill across the crypto stack; `uart_isr` (hw). The X25519 stack, SHA-256/SHA-512 families, and the QEMU-bring-up RNG are complete; the production ESP32-C6 RNG (HMAC-DRBG seeded from the on-chip TRNG) replaces the deterministic-fake at hardware bring-up time. `decode_u` removed from the registry — `field_unpack`'s limb 9 mask already drops bit 255 (the RFC 7748 high-bit mask), so a separate decode_u is redundant in our representation.
